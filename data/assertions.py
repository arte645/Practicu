...
class Assertions(Base):
    ...
    def to_be_hidden(self, locator, msg): #веб элемент скрыт
        loc = self.page.locator(locator)
        expect(loc).to_be_hidden(), msg
        
    def elements_count(self, locator, count: int, msg):
        loc = self.page.locator(locator)
        expect(loc).to_have_count(count), msg
